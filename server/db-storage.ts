...
export class DatabaseStorage implements IStorage {
  ...
  async updateCustomEntry(entryId: number, updates: Partial<{ customImageUrl: string; customArtist: string; customTags: string[]; userTags: string[]; keywords: string[]; rating: number }>): Promise<CustomEntry | undefined> {
  ...
  async createEntry(entryData: { title: string; imageUrl?: string; externalLink?: string; artist: string; tags?: string[]; userTags?: string[]; keywords?: string[]; type: 'comic' | 'image' | 'sequence' | 'story'; sequenceImages?: string[]; content?: string }): Promise<any> {
    ...
    if (entryData.userTags && entryData.userTags.length > 0) {
